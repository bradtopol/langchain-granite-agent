...
Coordinates multiple agents to complete complex research tasks
"""
...
class OpenClawOrchestrator:
    """
    Orchestrates multiple agents in a sequential workflow
    ...
    def __init__(
        ...
        # Initialize agents
        self.planner = PlannerAgent(model=model, base_url=base_url)
        self.researcher = ResearcherAgent(model=model, base_url=base_url)
        self.writer = WriterAgent(model=model, base_url=base_url)
    ...
    def check_ollama_connection(self) -> bool:
        ...
        if not self.planner.check_connection():
    ...
    def run_research(self, topic: str) -> OrchestrationResult:
        """
        ...
        print("\n" + "="*70)
        print("🔬 OpenClaw Research Orchestration")
        print("="*70)
        print(f"\nTopic: {topic}")
        ...
        if not self.check_ollama_connection():
            return OrchestrationResult(
                success=False,
                context=Context(original_request=topic),
                final_output="",
                execution_time=0,
        ...
        # Get final output (writer's output)
        writer_result = context.get_result("writer")
        final_output = writer_result.output if writer_result else ""
    ...
    def print_agent_info(self):
        ...
        print("\n" + "="*70)
        print("OpenClaw Orchestrator - Agent Information")
        print("="*70)
        print(f"\nModel: {self.model}")
